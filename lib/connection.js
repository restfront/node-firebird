(function () {
    ...
    function __onConnectionData(connection, data) {
        // Create new xdr reader, or get one from previous partial read
        let xdr;
        if (!connection._xdr) {
            ...
            const buf = new Buffer(data.length + xdr.buffer.length);
            xdr.buffer.copy(buf);
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            const operationCallback = connection._queue[0];

            let response;
            try {
                response = __decodeResponse(xdr, operationCallback, connection.db, connection._lowercase_keys);
                ...
                xdr.buffer = xdr.buffer = xdr.buffer.slice(xdrPos);
                xdr.pos = 0;
                connection._xdr = xdr;
                return;
            }
            ...
            if (response && response.status) {
                Messages.lookupMessages(response.status, (message) => {
                    ...
                    Utils.doCallback(response, operationCallback);
                });
            } else {
                Utils.doCallback(response, operationCallback);
            }
    ...
    function __decodeResponse(data, operationCallback, db, lowercase_keys) {
        // eat all dummy symbols from the start of response
        let operation;
        do {
            operation = data.readInt();
        } while (operation === Const.OP_dummy);

        // read differently by response type
        switch (operation) {
            case Const.OP_response:
                return __decodeSimpleResponse(data, operationCallback);

            case Const.OP_fetch_response:
                return __decodeFetchResponse(data, operationCallback, db, lowercase_keys);
            ...
            default:
                throw new Error('Unexpected response: ' + operation);
        }
    }

    function __decodeSimpleResponse(data, operationCallback) {
        const response = operationCallback && operationCallback.response || {};

        response.handle = data.readInt();
        const oid = data.readQuad();
        if (oid.low || oid.high) {
            response.oid = oid;
        }

        const buf = data.readArray();
        if (buf) {
            response.buffer = buf;
        }
        ...
        while (true) {
            ...
            switch (op) {
                ...
                case Const.ISC_arg_sql_state:
                    ...
                    const str = data.readString(Const.DEFAULT_ENCODING);
    ...
    function __decodeFetchResponse(data, operationCallback, db, lowercase_keys) {
        const RESULT_SET_EXHAUSTED = 100;
        let xdrPos = 0;

        const statement = operationCallback.statement;
        const transaction = operationCallback.transaction;
        ...
        if (data.fop) { // could be set when a packet is not complete
            data.readInt(); // eat "operation type" bytes
            data.fop = false;
        }
        ...
        while (data.fcount && (data.fstatus !== RESULT_SET_EXHAUSTED)) {
            for (let length = output.length; data.fcolumn < length; data.fcolumn++) {
                const item = output[data.fcolumn];
                try {
                    xdrPos = data.pos;
                    let value = item.decode(data);
                    if (custom.asObject) {
                        if (item.type === Const.SQL_BLOB) {
                            value = Blob.createBlobAsyncReader(statement, transaction, value, data.fcols[data.fcolumn]);
                        }
                        data.frow[data.fcols[data.fcolumn]] = value;
                    }
                    else {
                        if (item.type === Const.SQL_BLOB) {
                            value = Blob.createBlobAsyncReader(statement, transaction, value, data.fcolumn);
                        }
                    ...
                    data.pos = xdrPos;
                    data.r = Const.OP_fetch_response;
            ...
            if (custom.on) {
                custom.on(data.frow, statement.nbrowsfetched, output);
            }
        ...
        return {
            data: data.frows,
            fetched: Boolean(data.fstatus === RESULT_SET_EXHAUSTED)
        };
    }
