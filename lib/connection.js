(function () {
    ...
    function createSQLVar(type) {
        ...
        switch (type) {
            case Const.SQL_VARYING:
                return new Fields.SQLVarString();

            case Const.SQL_NULL:
                return new Fields.SQLVarNull();

            case Const.SQL_TEXT:
                return new Fields.SQLVarText();

            case Const.SQL_DOUBLE:
                return new Fields.SQLVarDouble();

            case Const.SQL_FLOAT:
            case Const.SQL_D_FLOAT:
                return new Fields.SQLVarFloat();

            case Const.SQL_TYPE_DATE:
                return new Fields.SQLVarDate();

            case Const.SQL_TYPE_TIME:
                return new Fields.SQLVarTime();

            case Const.SQL_TIMESTAMP:
                return new Fields.SQLVarTimeStamp();

            case Const.SQL_BLOB:
                return new Fields.SQLVarBlob();

            case Const.SQL_ARRAY:
                return new Fields.SQLVarArray();

            case Const.SQL_QUAD:
                return new Fields.SQLVarQuad();

            case Const.SQL_LONG:
                return new Fields.SQLVarInt();

            case Const.SQL_SHORT:
                return new Fields.SQLVarShort();

            case Const.SQL_INT64:
                return new Fields.SQLVarInt64();

            case Const.SQL_BOOLEAN:
                return new Fields.SQLVarBoolean();
    ...
    function prepareStatementParams(connection, transaction, params, input, callback) {
            ...
            connection.createBlob2(transaction, function (err, blob) {
                ...
                if (Buffer.isBuffer(b)) {
                    bufferReader(b, 1024, function (b, next) {
                        ...
                        ret[index] = new Fields.SQLParamQuad(blob.oid);
        ...
        for (var i = 0, length = params.length; i < length; i++) {
            ...
            if (meta.type === Const.SQL_BLOB && value != null) {
                ...
                ret[i] = createNullSQLParam(meta.type);
                done();
            } else {
                ret[i] = createSQLParam(value, meta.type);
                done();
            }
        }
    }
        ...
        switch (type) {
            ...
            case Const.SQL_TYPE_TIME:
                if (value instanceof Date) {
                    return new Fields.SQLParamDate(value);
                } else if (typeof(value) === 'string') {
                    return new Fields.SQLParamDate(Utils.parseDate(value));
                }

                return new Fields.SQLParamDate(new Date(value));
            ...
            case 'number':
                if (value % 1 === 0) {
                    if (value >= Const.MIN_INT && value <= Const.MAX_INT) {
                        return new Fields.SQLParamInt(value);
                    } else {
                        return new Fields.SQLParamInt64(value);
                    }
                }

                return new Fields.SQLParamDouble(value);

            case 'string':
                return new Fields.SQLParamString(value);

            case 'boolean':
                return new Fields.SQLParamBool(value);

            default:
                return new Fields.SQLParamString(value.toString());
    ...
    function createNullSQLParam(type) {
        switch (type) {
            ...
            case Const.SQL_NULL:
            case Const.SQL_TEXT:
                return new Fields.SQLParamString(null);

            case Const.SQL_DOUBLE:
            case Const.SQL_FLOAT:
            case Const.SQL_D_FLOAT:
                return new Fields.SQLParamDouble(null);

            case Const.SQL_TYPE_DATE:
            case Const.SQL_TYPE_TIME:
            case Const.SQL_TIMESTAMP:
                return new Fields.SQLParamDate(null);

            case Const.SQL_BLOB:
            case Const.SQL_ARRAY:
            case Const.SQL_QUAD:
                return new Fields.SQLParamQuad(null);

            case Const.SQL_LONG:
            case Const.SQL_SHORT:
            case Const.SQL_INT64:
            case Const.SQL_BOOLEAN:
                return new Fields.SQLParamInt(null);
    ...
    function decodeFetchResponse(data, callback, db, r) {
        var item, op, i, length, xdrpos;
                ...
                try {
                    ...
                    if (custom.asObject) {
                        if (item.type === Const.SQL_BLOB) {
                            value = fetchBlobAsync(statement, value, data.fcols[data.fcolumn]);
                        }
                        data.frow[data.fcols[data.fcolumn]] = value;
                    }
                    else {
                        if (item.type === Const.SQL_BLOB) {
                            value = fetchBlobAsync(statement, value, data.fcolumn);
                        }
        ...
        return {
            data: data.frows,
            fetched: Boolean(data.fstatus === 100)
        };
    }
